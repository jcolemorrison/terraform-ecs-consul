...
# 8500 defaults open, but we also need to open up 8501 for HTTPS API comm.
ports {
  http = 8500
  https = 8501
}
...
# these are turned off because the consul-ecs module doesn't work with them currently
...
config_entries {
  bootstrap = [
    ...
    },
    {
      ...
      Name = "${SERVICE_NAME_PREFIX}-payments"
      Sources = [
        {
    ...
    {
      Kind = "service-intentions"
      Name = "${SERVICE_NAME_PREFIX}-messages"
      Sources = [
        {
          Name = "${SERVICE_NAME_PREFIX}-client"
          Action = "allow"
    ...
    {
      Kind = "service-intentions"
      Name = "${SERVICE_NAME_PREFIX}-images"
      Sources = [
        {
          Name = "${SERVICE_NAME_PREFIX}-client"
          Action = "allow"
